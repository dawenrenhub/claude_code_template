...
2. **验证命令完整性 (Validation Commands)**
  AGENT.md 必须包含项目所需的全部验证命令，视项目类型可能包括：
  - 测试命令（如 npm test, pytest, go test 等）
  - Lint 命令（如 npm run lint, eslint, ruff 等）
  - 类型检查命令（如 npx tsc --noEmit, mypy 等）
  - 构建命令（如 npm run build, go build 等）
  通过查看项目的 package.json、pyproject.toml、Makefile、Cargo.toml 等配置文件来判断项目实际使用了哪些工具。
  如果项目尚未配置任何验证工具或为空仓，标注为 N/A，并给出“最小可行验证命令”建议（如占位的 lint/test 命令）。
  如果 AGENT.md 中缺少项目已配置的验证工具的对应命令，判定 FAIL。
...
6. **无遗漏环境前置条件 (No Missing Prerequisites)**
  如果项目运行验证命令前需要前置步骤（如安装依赖、初始化数据库、生成配置文件），AGENT.md 中应有说明。检查：
  - 是否需要先 npm install / pip install 但未提及
  - 是否需要环境变量（如 .env 文件）但未提及
